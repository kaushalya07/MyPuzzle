...
class Board (size:Int) {
    ...
    /**id the puzzle current arrangment of tile*/
    ...
    /**Return the 1-based index of the given place when all the places are arranged*/
    private fun indexOf(p:Place):Int{
        return  (p.y -1)*size + p.x
    }

    /**id this puzzle solved*/
    fun solved():Boolean{
        var result=true
        for(p:Place in places){
            result=result &&
    ...
    fun slide(tile: Tile?){
        for (p:Place in places){
            if (p.tile===tile){
                ...
                if (solved()){
                    notifyPuzzlesSolved(numOfMoves)
                }
                return
    ...
    /**is the tilt in the giveb place slideble*/
    ...
    private fun notifyPuzzlesSolved(numOfMove: Int){
        for (listener:BoardChangeListener in listeners){
